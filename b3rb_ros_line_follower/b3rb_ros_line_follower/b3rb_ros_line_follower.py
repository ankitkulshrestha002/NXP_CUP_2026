...
class LineFollower(Node):
    ...
    def get_sector_ranges(self, message, center_deg, half_width_deg):
        ...
        def to_index(deg):
            return int(
                round(
                    (math.radians(deg) - angle_min) /
                    angle_increment
                )
            ) % n

        i0 = to_index(center_deg - half_width_deg)
        i1 = to_index(center_deg + half_width_deg)

        sector = (
            message.ranges[i0:i1 + 1]
            if i0 <= i1
            else list(message.ranges[i0:]) +
            list(message.ranges[:i1 + 1])
        )

        return [
            r for r in sector
            if self.MIN_VALID_LIDAR_DIST < r < message.range_max
        ]
    ...
    def lidar_callback(self, message):
        ...
        # Measure distance to front and side structures
        self.zone_check_range = min(min_fc, min_fl, min_fr)

        # Fix for freezing: Only stop if obstacle is directly ahead (< 0.20m)
        self.obstacle_in_front = min_fc < self.FRONT_STOP_DIST

        # APF Shift calculation
        AVOID_DIST = 1.15
        MAX_SHIFT = 60.0
    ...
    def edge_vectors_callback(self, message):
        ...
        # SPATIAL SORTING: Force v1 to always be the Left line, v2 to be the Right line.
            ...
            if lane_width_top > (width * 0.38):
                ...
                if getattr(self, 'in_intersection', False):
                    ...
                    self.get_logger().info(
                        "🛣️ Lane safely realigned. Memory wiped for next target."
                    )
        ...
        dynamic_lookahead = 70.0 + (
            self.current_actual_speed * 40.0
        )
        ...
        raw_turn = (
            (kp_angle * theta_error) +
            (kd_angle * derivative)
        )

        smooth_turn = (
            (0.65 * self.target_turn) +
            (0.35 * raw_turn)
        )
    ...
    def resolve_hospital_payload(self, payload):
        if payload in SIGN_TO_HOSPITAL:
            return SIGN_TO_HOSPITAL[payload]

        if payload in SIGN_TO_HOSPITAL.values():
            return payload

        return None

    def resolve_patient_payload(self, payload):
        if payload in SIGN_TO_PATIENT:
            return SIGN_TO_PATIENT[payload]

        if payload in SIGN_TO_PATIENT.values():
            return payload
